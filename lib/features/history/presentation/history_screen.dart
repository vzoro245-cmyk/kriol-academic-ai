...
class _HistoryScreenState extends ConsumerState<HistoryScreen> {
  ...
  Future<Uint8List?> _getFileBytes(AcademicWork work) async {
    // 1. Tentar ler do arquivo local se o path estiver salvo e o arquivo ainda existir
  ...
  Future<void> _openWorkFile(AcademicWork work) async {
    ...
    try {
      ...
      if (work.localPath != null) {
        final file = File(work.localPath!);
        if (await file.exists()) {
          await OpenFilex.open(work.localPath!);
          return;
        }
      }
      ...
      if (bytes != null) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Não foi possível obter o arquivo para abrir.')),
          );
        }
      }
    } finally {
  ...
  Future<void> _saveWorkAs(AcademicWork work) async {
    ...
    try {
      ...
      if (bytes == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Não foi possível obter os dados do arquivo.')),
          );
        }
        return;
      }
